...
> [질문1] LIKE를 두 번 쓰기 싫은데 좀 더 fancy하게 풀 수 있는 방법이 뭐 없을까?
...
> 날짜 문제를 LIKE로 푸는 방법이 조금 습관으로 굳은 것 같은데, 
날짜 조건이 좀 더 섬세한 경우를 대비하여 BETWEEN이나 부등호로도 푸는 연습을 해야겠다!
...
> '모든 주문의 매출 합계'의 정의가 모호해서 아래의 계산식 중 고민했고, 모두 실행해보았습니다.
...
4번 계산식이 제일 아닐 줄 알았는데, 이게 정답이었네요 ㅎㅎ
문제에서 좀 더 명확히 정의를 해주었으면 좋았겠지만! 스스로 한번 더 문제에서 구하고자 하는 대상의 계산식을 계획하는 과정의 중요성을 생각해보았습니다 :)
...
> 오늘의 다짐: 습관적으로 INNER JOIN 대신 괜히 불안한 마음에 LEFT JOIN을 썼던 것 같은데, 무작정 LEFT JOIN을 쓰기보다 테이블 간의 관계를 명확히 이해하고 '나는 INNER JOIN한 데이터만 필요하다'는 확신과 계획을 가지고 쿼리를 짜야겠다!
...
> 앗 원택님이 Day 7에 남겨주신 row-oriented, column-oriented 링크를 오늘 문제를 풀고 나서 확인했습니다.

모든 컬럼을 조회한다고 하더라도 아래처럼 * 대신 모든 컬럼의 이름을 작성하는 것이 좋은 것인가요??

``` SELECT color, fixed_acidity, volatile_acidity, citric_acid, residual_sugar, chlorides, free_sulfur_dioxide, total_sulfur_dioxide, density, ph, sulphates, alcohol, quality ```
...
2. 고민 포인트: 모든 경기 =  올림픽인 줄 몰라서 '두 대회 연속 출전'을 깨닫는 시간
- 많은 경기 중에 올림픽 경기가 있는 줄 알았는데, 각 올림픽별 경기가 있었던 것.. 
- 그걸 모르고 처음에는 '경기ID로 두 대회 연속 출전을 식별하나?' 생각해서 처음에는 ROW_NUMBER + LAG로 접근해보았는데 조회해보니 원하는 결과를 출력해주지 않았음
- 설마 모든 경기 = 올림픽, 그래서 4년? 했는데 정답입니다 팝업이 뜸... 불친절한 문제.....
...
> 1. 테이블 확인해보니 메달 2개 이상 받은 선수 없음
- 따라서 GROUP_CONCAT과 그룹바이 없이 그냥 medals만 SELECT 해도 정답으로 처리가 되기는 함
...
> COALESCE에서 'critic_score IS NOT NULL' AND 조건까지 붙이는 방법을 처음에 생각했었는데 2011년부터 2015년까지를 복붙해보니 WHERE 조건으로 빼는 것이 더 깔끔하겠다고 판단하여 마지막에 그렇게 진행!

-- 이것도 쿼리 효율도 더 좋다고 함. 왜냐하면 CASE 안에 조건을 넣으면, DB는 전체 테이블을 스캔한 뒤 각 행에서 CASE 조건을 연산해야 하기 때문!
...
- id 라고 하니 당연히 FK겠지 하는 안일한 마음으로 조인했는데, 그리고 더 수정할 부분이 없어 보였는데 자꾸 정답이 아니라고 했다.
- 답답해 하던 중 원택님이 FK를 다시 확인해보라고 하셔서 테이블을 다시 확인해보았다.
- 머리도 뽀송하게 말리고 자리에 다시 돌아와서 테이블을 확인해보니 rental_id가 있었다.
- 생각해보니 고객이 중복해서 대여를 한 경우도 있을 터이고 실제로 그런 경우가 확인되었다.
- 정말 경솔했다!
...
- 처음에는 "해당 작가 작품들의 평균 리뷰 수"가 그냥 소설 / 비소설 불문하고 평균 리뷰 수인 줄 알아서 WHERE 조건 없이 쿼리를 짰음
- 그러나 생각할수록 소설의 평균 리뷰 수가 그렇게 상징적인건가? 수상한 작가들의 전체 장르 평균 리뷰 수랑 소설의 전체 평균 리뷰 수를 왜 비교하지? 싶어서 속는 셈 치고 WHERE 조건문 넣었더니 정답 확인
...
> 무난했으나 어제의 레슨런을 톡톡히 활용할 수 있었던 문제였다.

- FK는 artist_id가 아니라 artwork_id
...
> records 테이블로만 푼 쿼리도 공유해봅니다.

처음에는 row_number와 where 조건으로 접근했었는데, 제가 문제를 잘못 이해했다는걸 깨달았습니다. 첫 주문인 건들 중 count(distinct order_id)인데 quantity라고 착각했어요 -- 문제에 경고가 있었으나 그 말도 헷갈렸음. 상품의 수? 주문의 수?

아무튼 그렇게 quantity로 접근하다보니 계속 틀렸는데 테이블이 하나 더 있다는 사실을 깨닫고 조인해보면서 문제의 뜻을 이해하게 되었습니다.

테이블 조인으로 풀이 올리고, 그 다음에 처음에 접근했던 records 테이블로만 풀었던 쿼리를 완성시켜서 (테스트 통과) 함께 공유드려보아용!
...
-- 졸려서 + 4단계라고 하니 살짝 쫄아서 테이블 2개인 줄 모르고 records 테이블로만 푼 쿼리
...
WITH base AS (
    ...
    WHERE order_date IN (SELECT MIN(order_date) 
                        FROM records AS to_match 
                        WHERE records.customer_id = to_match.customer_id)
)
...
1. 베이스 테이블
- 처음에 베이스테이블에 100명의 친구를 가진 후보 조건을 넣어서 접근했다. 
- 그랬더니 베이스테이블로 기능하지 못하고, 후속 코드에서 괜히 edges 테이블을 계속 조인하게 되는 비효율을 범하게 되었다. 
- 그래서 베이스 테이블을 따로 빼게 되었다 (the table)


2. 후보 친구의 친구 수 구하기
- 처음에는 유저별 친구 수를 구하는(friend_cnt) CTE를 만들 생각을 못했다. 
- 'count한걸 어떻게 sum 하지?' 하는 고민을 하다가 결국 분리해보았는데, 분리했더니 수월해졌다.
- 'friend_cnt를 참조해서 SUM을 하면 되는구나!'를 깨달았을 때 희열이 있었다. (사실 GPT한테 조언을 좀 구했다)
...
> customer_id가 유니크하지 않기 때문에 GROUP BY 필요!
...
> ## TO_CHAR을 활용해서 weekday를 짜고 싶었기 때문에 postgreSQL로 풀었습니다!

1. 오해 포인트
- 주문건수가 물량 수의 SUM의 개념인 줄 알았는데 단순 횟수 COUNT 였다.

2. 학습 포인트
- TO_CHAR(calendar.order_date, 'Day') AS weekday 로 풀게 되면 'Thrusday ' 이렇게 공백이 생긴다.
- 따라서 'Day'가 아닌 'FMDay'로 풀어야 함
- LAG (컬럼, n, 0) 이렇게 null에 대한 처리도 할 수 있다는 점
...
> SELECT level에서 집계함수 AVG와 CNT를 함께 쓸 수 없음! -- 따라서 CTE로 구분 필요
...
- 처음에 letsrank에서 city_id를 SELECT하지 않고, 메인 쿼리에서 base랑 조인을 함 (base에 city_id가 있기 때문에)
- 단순히 'city_id 값을 가져와야지' 생각했지 둘을 customer_id로 조인했을 때 도시별 ROW를 집계한 값이 와해될 것을 예상하지 못함
- letsrank에서 city_id SELECT 하고 메인 쿼리에서 조인하지 않음으로써 문제 해결
